...
template<class T>
Polynomial<T>::Polynomial(const Polynomial<T>& polynom) : size__(polynom.size__) {
	data__ = new T[size__];
	for (size_t i = 0; i < polynom.size__; ++i)
		data__[i] = polynom.data__[i];
}
template <class T>
